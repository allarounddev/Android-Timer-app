package com.forixusa.scoretimerdat.android.graph;
...
public class GraphView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		final float datalength = values.length;
		final float colwidth = (width - 2 * border) / datalength;
		final float halfcol = colwidth / 2;
		float lasth = 0;
		for (int i = 0; i < datalength; i++) {
			final float val = values[i] - min;
			final float rat = val / diff;
			final float h = graphheight * rat;
			if (i > 0) {
				canvas.drawLine((i - 1) * colwidth + horstart + 1 + halfcol, border - lasth + graphheight, i * colwidth
						+ horstart + 1 + halfcol, border - h + graphheight, paint);
			} else {
				canvas.drawLine(border + 3, border - lasth + graphheight, i * colwidth + horstart + 1 + halfcol, border
						- h + graphheight, paint);
			}
			lasth = h;
		}
