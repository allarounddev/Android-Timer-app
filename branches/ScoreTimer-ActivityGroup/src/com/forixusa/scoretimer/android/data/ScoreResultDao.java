package com.forixusa.scoretimer.android.data;
...
public class ScoreResultDao extends DBDAOBase {
	...
	private String [] mColumns= new String[] {
			DBConstraint.SCORE_RESULT_ID,
			DBConstraint.DATE,
		    DBConstraint.ACCURACY,
		    DBConstraint.PACE,
		    DBConstraint.ESTIMATION_SCORE
			};
	...
	public ScoreResult select(final int id) {
	    ...
	    Cursor cursor = null;
	    try {	  
		   cursor = this.db.query(DBConstraint.SCORE_RESULT_TABLE_NAME, mColumns , DBConstraint.SCORE_RESULT_ID +  " MATCH ?", new String[] {Integer.toString(id)}, null, null, null);		      
	...
	public long update(ScoreResult scoreResult){	
		return this.db.update(DBConstraint.SCORE_RESULT_TABLE_NAME, createContent(scoreResult),DBConstraint.SCORE_RESULT_ID + " MATCH ?", new String[] {String.valueOf(scoreResult.scoreResultId)});
	}
