...
public class TabActivityGroup extends ActivityGroup {
    ...
    public void finishFromChild(Activity child) {
        ...
        if (length > 1) {
            ...
            if (!mActivityID.isEmpty()) {
              ...
              //  mIntent.get(newlength - 1).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

               // Log.d(TAG, "mActivityID.get(newlength - 1):" + mActivityID.get(newlength - 1));
             //   final LocalActivityManager manager1 = getLocalActivityManager();
                //final Activity activity = manager.getActivity(mActivityID.get(newlength - 1));
               // activity.onContentChanged();
            }
    ...
    public void startChildActivity(String id, Intent intent) {
        if (mActivityID.contains(id)) {
            int index = mActivityID.indexOf(id);
            LocalActivityManager manager = getLocalActivityManager();
