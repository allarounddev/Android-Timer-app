...
public class LineChartView extends View {
	...
	private static final float X_RIGHT = ScoreTimerApplication.getWidthScreen() - 70;
		...
		for (i = 0; i <= Math.round(topYLargest); i++) {
			if (i == Math.round(topYLargest)) {
				...
				if (temp.length() < 3) {
					...
					canvas.drawText(temp, 3, Y_BOTTOM - i * yIncrement + 5, mPaint);
				}
			}
		}
			...
			for (int j = 0; j <= Math.round(topYSmallest); j++) {
				if (j == Math.round(topYSmallest)) {
					if (String.valueOf(topYSmallest).length() < 3) {
						...
						canvas.drawText(String.valueOf(topYSmallest), 17, Y_BOTTOM - j * yIncrement + 5, mPaint);
					} else {
						canvas.drawText(String.valueOf(topYSmallest), 3, Y_BOTTOM - j * yIncrement + 5, mPaint);
					}
				}
			}
		}
